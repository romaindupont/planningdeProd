...
const Form = ({ transfertRefToValo, updatedProgress, changePopup, popupWindow, addToValo, choraire }) => {
  const transfertClic = (e) => {
    ...
    if (e.target.cout.value !='') {
      ...
      transfertRefToValo(3, e.target.planning_id.value, e.target.reference.value, e.target._end.value, e.target.quantity.value, e.target.cout.value, montantTt);
