...
const Tbody = ({ search, tableauValo, dateOne, dateTwo, setFilterList, waitValo }) => {
  let filterList = tableauValo;
  ...
  if (search != '') {
    filterList = tableauValo.filter((list) => list.name.includes(search))
  }
  useEffect(() => {
  ...
  return (
    <tbody>
      {waitValo && (<tr key="patience" className="app-load"><td value="">Veuillez patienter</td></tr>)}
      {!waitValo && (filterList.map((valo, i) =>
